...
class yolo_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_result, renormalize, task, img_size, mask_dir=None, use_mask_constraint=False, mask_blur=7, mask_power=1.0):
    ...
    def get_mask_path(self, img_path):
        ...
        mask_path = os.path.join(self.mask_dir, f'{stem}.txt')
        return mask_path if os.path.exists(mask_path) else None

    def load_segmentation_mask(self, img_path, original_shape, letterbox_ratio, padding):
        mask_path = self.get_mask_path(img_path)
        if mask_path is None:
            return None

        original_h, original_w = original_shape[:2]
        target_mask = np.zeros((self.img_size, self.img_size), dtype=np.uint8)
        ratio_w, ratio_h = letterbox_ratio
        ...
        try:
            ...
            print(f'Warning... {mask_path} read failure.')
            return None
        ...
        for line in lines:
            ...
            for x, y in zip(coords[0::2], coords[1::2]):
                px = x * original_w * ratio_w + left
                py = y * original_h * ratio_h + top
                px = int(round(np.clip(px, 0, self.img_size - 1)))
                py = int(round(np.clip(py, 0, self.img_size - 1)))
                points.append([px, py])
            if len(points) >= 3:
        ...
        cv2.fillPoly(target_mask, polygons, 255)
        if self.mask_blur and self.mask_blur > 0:
            ksize = int(self.mask_blur)
            if ksize % 2 == 0:
                ksize += 1
            target_mask = cv2.GaussianBlur(target_mask, (ksize, ksize), 0)

        target_mask = target_mask.astype(np.float32) / 255.0
        if self.mask_power and self.mask_power != 1.0:
            target_mask = np.power(target_mask, self.mask_power)
        return target_mask

    def renormalize_cam_in_bounding_boxes(self, boxes, image_float_np, grayscale_cam, segmentation_mask=None):
        """Normalize CAM inside detection boxes; optionally constrain it with segmentation mask."""
        ...
        for x1, y1, x2, y2 in boxes:
            ...
            roi_cam = grayscale_cam[y1:y2, x1:x2].copy()
            roi_cam = scale_cam_image(roi_cam)

            if segmentation_mask is not None:
                roi_mask = segmentation_mask[y1:y2, x1:x2].copy()
                if roi_mask.max() > 0:
                    roi_cam = roi_cam * roi_mask
                    roi_cam = scale_cam_image(roi_cam)
                else:
                    roi_cam = np.zeros_like(roi_cam, dtype=np.float32)

            renormalized_cam[y1:y2, x1:x2] = np.maximum(renormalized_cam[y1:y2, x1:x2], roi_cam)
    ...
    def process(self, img_path, save_path):
        ...
        img, ratio, (top, bottom, left, right) = letterbox(img, new_shape=(self.img_size, self.img_size), auto=True) # 如果需要完全固定成宽高一样就把auto设置为False
        segmentation_mask = None
        if self.use_mask_constraint:
            segmentation_mask = self.load_segmentation_mask(img_path, original_shape, ratio, (top, bottom, left, right))
...
def get_params():
    params = {
        # 'weight': 'runs/xiaorong_module/yolo11/weights/best.pt', # 现在只需要指定权重即可,不需要指定cfg
        # 'weight': 'runs/xiaorong_module/GEIT_DIMB_SEAM_FMIOU/yolo11/weights/best.pt',
...
if __name__ == '__main__':
    ...
    model(r'heatmap/DDSM-MIAS-V2.0/val/images/A_1021_1.RIGHT_MLO.jpg', 'heatmap/output/YOLO12/A_1021_1.RIGHT_MLO')
